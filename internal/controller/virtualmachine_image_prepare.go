...
func (r *VirtualMachineReconciler) EnsureImageOnProvider(
	...
	// Image prepare is a PRE-CREATE step: run it only when this reconcile would
	// actually create the VM, mirroring createVM's gate (Status.ID unset AND not
	// adopted — see virtualmachine_controller.go ~line 299). An already-created VM
	// (Status.ID set) has already consumed its image, so preparing it now is wasted
	// work; an adopted/cloned VM's disk is created out-of-band by the adoption/clone
	// controller and never needs a prepare. Skipping both keeps existing VMs exactly
	// as before this feature.
	if vm.Status.ID != "" || vmIsAdopted(vm) {
		return false, nil
	}
