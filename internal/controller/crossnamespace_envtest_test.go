...
var _ = Describe("Cross-namespace VMClone target (envtest)", func() {
	...
	// The refusal rechecks only every crossNamespaceRecheckInterval (minutes),
	// so a clone that completes within this spec's timeout proves the
	// SetupWithManager Namespace watch (predicate + mapping) re-drove it the
	// moment the grant was added.
	It("re-drives a refused clone through the Namespace watch when the grant is added", func() {
		mgrCtx, stop := context.WithCancel(ctx)
		DeferCleanup(stop)

		src := &corev1.Namespace{ObjectMeta: metav1.ObjectMeta{GenerateName: "xnsw-src-"}}
		tgt := &corev1.Namespace{ObjectMeta: metav1.ObjectMeta{GenerateName: "xnsw-tgt-"}}
		Expect(k8sClient.Create(ctx, src)).To(Succeed())
		Expect(k8sClient.Create(ctx, tgt)).To(Succeed())
		...
		Expect(migrations.SetupWithManager(mgr)).To(Succeed())
		...
		clone := &infravirtrigaudiov1beta1.VMClone{
			ObjectMeta: metav1.ObjectMeta{Name: "clone-w", Namespace: src.Name},
			Spec: infravirtrigaudiov1beta1.VMCloneSpec{
				Source: infravirtrigaudiov1beta1.CloneSource{VMRef: &infravirtrigaudiov1beta1.LocalObjectReference{Name: "src-vm"}},
				Target: infravirtrigaudiov1beta1.VMCloneTarget{Name: "web", Namespace: tgt.Name},
			},
		}
		Expect(k8sClient.Create(ctx, clone)).To(Succeed())

		By("the running controller refuses the clone")
		Eventually(func(g Gomega) {
			got := &infravirtrigaudiov1beta1.VMClone{}
			g.Expect(k8sClient.Get(ctx, client.ObjectKeyFromObject(clone), got)).To(Succeed())
			ready := readyCondition(got.Status.Conditions, infravirtrigaudiov1beta1.VMCloneConditionReady)
			g.Expect(ready).NotTo(BeNil())
			g.Expect(ready.Reason).To(Equal(ReasonTargetNamespaceNotAllowed))
		}, "20s", "200ms").Should(Succeed())

		By("granting the target namespace re-drives it without waiting for the recheck")
		Expect(k8sClient.Get(ctx, client.ObjectKeyFromObject(tgt), tgt)).To(Succeed())
		tgt.Annotations = map[string]string{AllowedSourceNamespacesAnnotation: src.Name}
		Expect(k8sClient.Update(ctx, tgt)).To(Succeed())
		Eventually(func(g Gomega) {
			got := &infravirtrigaudiov1beta1.VMClone{}
			g.Expect(k8sClient.Get(ctx, client.ObjectKeyFromObject(clone), got)).To(Succeed())
		...
		Expect(k8sClient.Get(ctx, client.ObjectKey{Namespace: tgt.Name, Name: "web"}, &infravirtrigaudiov1beta1.VirtualMachine{})).To(Succeed())
	})
})
