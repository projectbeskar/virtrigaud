...
// ListHosts is the gRPC host-inventory RPC (ADR-0007 P1). The mock provider does
// not model a clustered host set, so — like the production single-host providers
// — it reports Unimplemented and advertises supports_clustering = false (D7,
// honesty-first).
...
// on the mock provider for the same reason as ListHosts.
