...
func (v *VirshProvider) setupConnection() error {
	...
	// Set SSH password via environment for password authentication
	if v.credentials.Password != "" {
		v.env = append(v.env, fmt.Sprintf("SSH_ASKPASS_REQUIRE=never"))
		v.env = append(v.env, fmt.Sprintf("SSHPASS=%s", v.credentials.Password))
		log.Printf("INFO Configured password authentication via environment")
	}
...
func (v *VirshProvider) runVirshCommand(ctx context.Context, args ...string) (*VirshResult, error) {
	...
	command := strings.Join(append([]string{"virsh"}, args...), " ")
