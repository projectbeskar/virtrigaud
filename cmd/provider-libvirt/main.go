...
func main() {
	var (
		...
		providerType     = flag.String("provider-type", "libvirt", "Provider type")
	...
	setupLog.Info("Starting libvirt provider server",
		"grpc-addr", *grpcAddr,
	...
	// Create a fake Provider object for the libvirt provider
	...
	// Create the libvirt provider instance
	ctx := context.Background()
	provider, err := libvirt.NewProvider(ctx, k8sClient, providerObj)
	if err != nil {
		setupLog.Error(err, "unable to create libvirt provider")
		os.Exit(1)
	}
