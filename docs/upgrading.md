...
| **`VMImage` prepare state is keyed by Provider `<namespace>/<name>`** ([`docs/image-preparation.md`](image-preparation.md#prepare-state-is-per-provider)) | `status.providerStatus` and `status.availableOn` use `<namespace>/<name>` instead of the bare Provider name; each entry records the Provider's UID and its own `taskRef`; the image-wide `status.prepareTaskRef` is no longer written. On each image's first prepare reconcile, a bare-name entry is moved to the image's own namespace when a Provider of that name exists there (and re-validated once through the idempotent prepare), otherwise dropped; a legacy `prepareTaskRef` is cleared. | Upgrade the `VMImage` CRD **before** the manager (readiness fails on a CRD without `providerStatus[].providerUID`/`taskRef`). Update scripts or out-of-band preparers that read or write `providerStatus`/`availableOn`. Expect one `ImagePrepare` call per image and Provider after the upgrade (a no-op when the prepared image exists). |
