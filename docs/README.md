...
| [`docs/cross-namespace-references.md`](cross-namespace-references.md) | Cross-namespace `Provider` / `VMClass` / `VMImage` references: `spec.consumerNamespaceSelector` (unset = own namespace only, any empty selector = every namespace, label selectors, `kubernetes.io/metadata.name`, negative selectors), why sharing a `Provider` shares everything its credentials reach, who can grant (object owners, namespace labellers, self-service platforms), the `ConsumerNotAllowed` refusal and what each controller checks (class and image grants of an existing VM only where used), how a grant takes effect (indexed watches, 5-minute recheck), revocation, deleting a VM whose `Provider` is refused (orphan-on-delete / force-delete), the CRD readiness check, and upgrade notes (breaking: CRDs, then selectors, then the manager) |
