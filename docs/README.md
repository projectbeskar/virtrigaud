...
| [`docs/cross-namespace-targets.md`](cross-namespace-targets.md) | Cross-namespace `VMClone` / `VMMigration` targets: the admin-set `infra.virtrigaud.io/allowed-source-namespaces` grant, the `TargetNamespaceNotAllowed` refusal and how to recover from it, what revoking a grant stops, why `VMMigration` `spec.source.providerRef` must name the source VM's own Provider, and upgrade notes |
