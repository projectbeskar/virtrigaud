...
- `internal/providers/contracts/image.go`, `capabilities.go`: `ImagePrepareRequest.Image`/`SourceDigest`/`Provider`, `ImagePrepareResponse.Artifact`, `PreparedArtifact`, `ImagePrepareResponse.ConfirmsIdentity` (the D7 echo check: UID and digest must both match), `Capabilities.SupportsImageArtifactIdentity`.
- `internal/transport/grpc/client.go`: maps the new request fields (an identity without a UID is never sent), the artifact echo and the capability.
- `internal/imageartifact/` (new): `SourceDigest` (D2: `sha256:` over a versioned, key-sorted canonical JSON of `spec.source` with `http.timeout`/`headers`/`authentication` cleared; golden-vector pinned), `ValidateSourceDigest`, `ArtifactName` (D1: `<ns>.<name>` cut to the budget + separator + 16 hex of `sha256("v1/"+uid+"/"+digest)`; vSphere 80/`_`, libvirt 200/`_`, Proxmox 80/`-`), `Stamp` and `Decide` (D3/D4 fail-closed reuse table), `ParseRequest` (D7 identity vs. deprecated legacy mode, strict validation), `ConflictError`/`InProgressError`, and `SignalLegacyRequest` (WARN log + counter).
...
- Docs: `docs/image-preparation.md` (the new contract and status field; the limitation stands until the later slices), `docs/upgrading.md` (readiness fields).
