...
- `go.mod`: `golang.org/x/crypto` promoted from `// indirect` (v0.55.0, pulled in via `minio-go`) to **direct**, at the same version — `go.sum` has a **zero-byte diff**, confirming this adds no new module tree.
