...
- Grants take effect within seconds: the VirtualMachine, VMSnapshot, VMClone and VMMigration controllers watch `Namespace` label changes and the `spec.consumerNamespaceSelector` of Providers, VMClasses and VMImages (status-only updates never pass), re-driving refused objects (and, for VMs, every VM with a cross-namespace reference, so a revocation is noticed promptly too). The 5-minute recheck is only a safety net.
