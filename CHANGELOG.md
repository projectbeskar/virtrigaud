...
> **Operator note.** The vSphere provider now advertises `supportsImageArtifactIdentity`. A prepare request that carries the `VMImage` identity (the Slice 2 manager) imports the OVA as a template named `<namespace>.<name>_<16 hex>` into the Provider's import folder (`spec.defaults.folder`, or the datacenter VM folder when it is empty; a configured folder that does not resolve fails the prepare, retried, never a fallback — check that each Provider's `defaults.folder` resolves to exactly one VM folder) and reuses an existing template there only when its `virtrigaud.image.*` stamp matches the image UID and source digest; anything else at that name is a Conflict and is never touched. `status.providerStatus[...].id` becomes the template's absolute inventory path. Templates prepared before this change are left alone, and the first create after the upgrade imports each image once more. **Trust assumption:** nobody but the Provider's vCenter account may create, move, rename, reconfigure (AdvancedConfig) or mark as template VMs in the import folder (folder permissions propagate) — use a dedicated folder with a restrictive ACL. **Security:** a bare `.ovf` could make the provider upload any file its pod can read (pre-existing since #154, in v0.3.11): OVF file references must now be plain member names of the OVA, identity mode refuses bare `.ovf` sources, and legacy mode refuses a bare `.ovf` that references any file — republish such images as `.ova`. Image downloads refuse loopback/link-local/unspecified/multicast destinations, follow at most 3 redirects and stop at `VIRTRIGAUD_VSPHERE_IMAGE_MAX_DOWNLOAD_GIB` (new, default 256; set it through `spec.runtime.env`). Image-source failures no longer count toward the Provider's circuit breaker. Requests from an older manager are served the pre-ADR way for this release, with a `WARN` log and `virtrigaud_provider_image_prepare_legacy_requests_total{provider_type="vsphere"}` (the vSphere provider now serves `/metrics` on its health port): alert on it. `DuplicateName` behaviour verified on vCenter 8.0.2. No new vCenter privilege.

### Security
- `internal/providers/vsphere/ova_archive.go` (new), `ova_import.go`, `image.go`: **arbitrary local file read → vCenter upload, pre-existing since #154 and present in v0.3.11.** A bare `.ovf` source was imported through govmomi's `importer.FileArchive`, which opens every `<References><File ovf:href>` as a path on the provider pod's filesystem (absolute as-is, relative joined to the staging directory, `../` escaping), and an `.ova` through `TapeArchive`, which matches hrefs with `path.Match`. A tenant-written OVF could reference the mounted vCenter credentials or TLS key as a CD-ROM ISO and the provider uploaded it into the template, readable from every clone. Now every href is validated before `CreateImportSpec` in both legacy and identity mode (a plain file name: no `/`, `\`, `.`/`..`, `:`, `*?[`, control characters, at most 255 bytes) and must be a root-level member of the package; the importer reads only through `ovfPackage`, which serves the descriptor and the validated names by exact member name from the staged download and never opens a file named by the OVF. Identity mode refuses a bare `.ovf` source; legacy mode refuses a bare `.ovf` that references any file (one that references none still imports). A refusal is `InvalidSpec` (the legacy path now returns it as such instead of `Internal`). The core fix is its own commit, suitable for a v0.3.x backport.
- `internal/providers/vsphere/image_download.go` (new), `image.go`: **SSRF and unbounded downloads from the provider pod (pre-existing).** `http.DefaultClient` is replaced by a client whose dialer refuses loopback, link-local (including `169.254.169.254`), unspecified (`0.0.0.0/8`, `::`) and multicast addresses on every connection after DNS (RFC 1918 stays allowed), whose `CheckRedirect` re-checks scheme and address and caps redirects at 3, and which bounds TLS-handshake and response-header waits; IP-literal and localhost targets are refused before the request. Downloads stop at `VIRTRIGAUD_VSPHERE_IMAGE_MAX_DOWNLOAD_GIB` (default 256, 1–16384; invalid → default + WARN): larger images are `InvalidSpec`. Returned messages no longer quote HTTP codes beyond "an HTTP client error", transport errors, tar/XML parser output, vCenter fault text or the computed checksum (logged instead), so a status is no oracle for what an arbitrary URL serves; `redactURL` keeps only scheme, host and last path segment.
- `internal/providers/contracts/errors.go`, `internal/imageartifact/request.go`, `internal/transport/grpc/client.go`, vSphere: **one tenant's failing image source tripped the Provider-wide circuit breaker.** New ErrorInfo reason `IMAGE_SOURCE_UNAVAILABLE` (`imageartifact.SourceUnavailableError`); `countsTowardBreaker` ignores it on `ImagePrepare`, like `IMAGE_ARTIFACT_IN_PROGRESS`, and it stays retryable. vSphere tags source 5xx/408/429, unreachable or broken-off sources, unstageable downloads and vCenter refusing an image's content with it; only vCenter session, rights and transport failures still count. Raw error paths (datastore/pool resolution, local read-back) go through fixed, sanitized messages. (The libvirt provider adopts the reason in a follow-up.)
- Docs: ADR-0009 threat row G no longer claims "denial of service at worst": once a VMImage exists, a principal with create/move/rename, AdvancedConfig and MarkAsTemplate rights in the import folder can plant a matching template that is reused (and can get the provider to destroy a powered-off VM stamped as this image's abandoned import). The ADR and `docs/image-preparation.md` state the trust assumption and recommend a dedicated import folder with a restrictive ACL (a separate `defaults.imageFolder` is a tracked follow-up).

### Added
- `internal/providers/vsphere/image_identity.go`: identity-mode `ImagePrepare` (ADR-0009 D1-D6). D1 name via `imageartifact.ArtifactName(NameRuleVSphere)`; strict import-folder resolution (`resolveArtifactFolder`: fall back to the datacenter VM folder only when `DefaultFolder` is empty; a folder that is missing, ambiguous or outside the datacenter VM folder is `FailedPrecondition` — retried by the manager but not counted toward its circuit breaker — and a vCenter error is `Unavailable`); folder-scoped probe (`vmsNamedInFolder`, no finder); the D4 decision with vSphere liveness (a queued or running task in `recentTask` — an active import shows `ResourcePool.ImportVAppLRO` there on vCenter 8.0.2 — or a task that cannot be read (fail closed), `Destroy_Task` in `disabledMethod` as a defensive extra, or a `preparedat` younger than `max(2 × spec.prepare.timeout, 2h)` with the timeout capped at one year; only a powered-off, non-template, matching, non-live object is removed as abandoned, re-verified right before the destroy); `DuplicateName` — reported through the NFC lease on vCenter 8.0.2, handled synchronously too — re-runs the probe, and a holder the probe cannot see (a vApp, a folder, a case-differing VM name) is a Conflict instead of an import loop; lowest-MOID convergence after the import as a fallback (a call whose own object is not the lowest destroys its own) and a template is handed out only while its name is unique in the folder (otherwise, and when the name does not settle, `imageartifact.InProgressError`); `prepared_image_id` is the absolute inventory path, checked to resolve (`FindByInventoryPath`) to the verified template; the artifact echo is the stamp; a per-call finder, so concurrent prepares do not race on the shared one; lease aborts and the cleanup of this call's unfinished import run on detached contexts (vCenter deletes the entity of an aborted lease; a cleanup that finds it gone succeeds).
- `internal/providers/vsphere/image_artifact.go`: the `virtrigaud.image.{stampversion,uid,namespace,name,sourcedigest,preparedby,preparedat}` ExtraConfig stamp (parse fails closed on a repeated key, a non-string value, an unknown version, a malformed UID or digest, or a non-RFC 3339 `preparedat`), the cleared-stamp entries, the staleness bound, MOID ordering, and the folder-level decision over every object at the name.
- `internal/providers/vsphere/server.go`: `GetCapabilities` advertises `SupportsImageArtifactIdentity`; `New` reads `VIRTRIGAUD_VSPHERE_IMAGE_MAX_DOWNLOAD_GIB`.
- `cmd/provider-vsphere/main.go`: serves the provider's Prometheus registry at `/metrics` on the health port, as libvirt and Proxmox do, so the legacy-request counter can be scraped.
- Tests (vcsim): fresh stamped import and echo, reuse without download, new UID/digest → new artifact, conflicts left untouched (unstamped, other UID/digest, untrusted stamp, unfinished foreign VM, stale idle VM of another image or source), unfinished artifacts (fresh, running task, unreadable task, `Destroy_Task` blocked, `prepare.timeout`-derived bound, powered on, abandoned → re-import), import-folder scope and unusable folders, multi-VM OVF, permanent vs transient vs image-source failure classification, parser output and URL secrets never reaching errors or logs, an OVF-forged `virtrigaud.image.*` stripped, vcsim's missing `DuplicateName` pinned, emulated `DuplicateName` through the lease and synchronously (reuse / in progress / conflict), a name held out of sight (folder, case-differing VM) and a vanished holder, lowest-MOID convergence both ways and under 4 concurrent prepares, `Create`/`Clone` clearing the stamp, `Create` cloning the verified artifact by inventory path next to a same-named decoy, legacy mode with the deprecation signal, the capability, the manager transport's error typing over gRPC; OVF reference attacks (absolute, traversal, URL, `file:`, glob, backslash) refused in both modes with no `CreateImportSpec` and no sentinel file in any NFC upload, bare `.ovf` handling, OVAs with member files importing exactly those members; download address checks, dial hook, redirects, size limit, env parsing. Transport: the breaker stays closed across repeated `IMAGE_SOURCE_UNAVAILABLE` answers over bufconn (a plain `Unavailable` still trips it).

### Changed
- `internal/providers/vsphere/image.go`: `ImagePrepare` decodes requests with `imageartifact.ParseRequest` before any vCenter call: an identity request goes to the new path; an identity-less request from an older manager is served by `imagePrepareLegacy` (the pre-ADR bare-name path: bare-name gate across the datacenter, `prepared_image_id` = bare name, no echo, unstamped — with the security fixes above) and emits `imageartifact.SignalLegacyRequest` (deprecated; refused from the next release). A legacy target name must now be a DNS-1123 subdomain, as every `VMImage` name is.
- `internal/providers/vsphere/ova_import.go`: in identity mode `importOVA` appends the image stamp to the import spec after `stripReservedExtraConfig` and before `ImportVApp`, refuses an OVF that is not exactly one VM as `InvalidSpec`, classifies an unreadable/invalid OVF or an OVF parse/validation fault from vCenter as `InvalidSpec` (transient OVF faults — `OvfSystemFault`, consumer callback faults, `OvfImportFailed` — stay retryable), maps `DuplicateName` to a re-probe, and returns the created entity on a late failure so the caller destroys its own object.
- `internal/providers/vsphere/server.go`: `Create` and `Clone` clear `virtrigaud.image.*` on the new VM (vSphere removes an ExtraConfig key set to `""`), so a clone of a prepared template is never an image artifact.
- Docs: `docs/image-preparation.md` (new "vSphere: identity-safe prepared templates" section: naming, import folder and trust assumption, stamp, package and download restrictions, reuse table, liveness, concurrency, "Verified on vCenter 8.0.2", failure classification, legacy mode, legacy templates, privileges; "Provider support" lists vSphere), `docs/vm-ownership.md` (legacy vs identity `ImagePrepare`, OVF-forged image stamps, clones clear the image stamp, no new privilege, reference-style known limitation → Slice 7), `docs/upgrading.md` (vSphere rows: prepared templates, OVF packages and bare `.ovf`, download restrictions and the new env var; old-manager rollback caveat), `docs/adr/0009-prepared-image-artifact-identity.md` (D4/D6 vCenter 8.0.2 findings, threat row G, status: Slices 1–5 implemented) and `docs/adr/README.md`.

### Why
ADR-0009 closes cross-tenant template poisoning and disclosure on vSphere: a prepared template is named after, stamped with and reused only for the `VMImage` UID and source digest it was built for, in one folder, and anything else at the name is refused. The security review found a pre-existing local file read through bare OVF references, an SSRF-capable unbounded download, and tenant-triggerable breaker trips; all three are fixed here. A synchronous import that can never succeed now tells the manager so (`InvalidSpec`), so the image is held instead of re-downloaded every few seconds.

### Impact
- [ ] Breaking change (bare `.ovf` sources with file references and images served from loopback/link-local addresses are refused)
- [x] Requires cluster rollout (vSphere provider image; takes effect with the Slice 2 manager; the breaker exclusion needs this manager)
- [ ] Config change only
- [ ] Documentation only
