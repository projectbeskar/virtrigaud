...
## [2026-09-25 19:03] - ADR-0009 Slice 3: identity-safe image preparation on vSphere
**Author:** @wrkode (William Rizzo)

> **Operator note.** The vSphere provider now advertises `supportsImageArtifactIdentity`. A prepare request that carries the `VMImage` identity (a manager with ADR-0009 Slice 2) imports the OVA as a template named `<namespace>.<name>_<16 hex>` into the Provider's import folder (`spec.defaults.folder`, or the datacenter VM folder when it is empty; a configured folder that does not resolve fails the prepare, retried, never a fallback — check that each Provider's `defaults.folder` resolves to exactly one VM folder) and reuses an existing template there only when its `virtrigaud.image.*` stamp matches the image UID and source digest; anything else at that name is a Conflict and is never touched. `status.providerStatus[...].id` becomes the template's absolute inventory path. Templates prepared before this change are left alone, and the first create after the upgrade imports each image once more. For multi-tenant setups, give each tenant's Provider its own import folder. Requests from an older manager are still served the pre-ADR way for this release, with a `WARN` log and `virtrigaud_provider_image_prepare_legacy_requests_total{provider_type="vsphere"}` (the vSphere provider now serves `/metrics` on its health port): alert on it. **Merge gate:** that `ImportVApp` returns `DuplicateName` for a taken name must still be confirmed on a real vCenter (vcsim does not model it; the lowest-MOID fallback is implemented and tested). No new vCenter privilege.

### Added
- `internal/providers/vsphere/image_identity.go`: identity-mode `ImagePrepare` (ADR-0009 D1-D6). D1 name via `imageartifact.ArtifactName(NameRuleVSphere)`; strict import-folder resolution (`resolveArtifactFolder`: fall back to the datacenter VM folder only when `DefaultFolder` is empty; a folder that is missing, ambiguous or outside the datacenter VM folder is `FailedPrecondition` — retried by the manager but, unlike `Unavailable`, not counted toward its circuit breaker, so a wrong `defaults.folder` cannot stop the Provider's other RPCs — and a vCenter error is `Unavailable`); folder-scoped probe (`vmsNamedInFolder`, no finder); the D4 decision with vSphere liveness (a queued/running task in `recentTask`, `Destroy_Task` in `disabledMethod` as while an NFC lease holds the entity, or a `preparedat` younger than `max(2 × spec.prepare.timeout, 2h)`; only a powered-off, non-template, matching, non-live object is removed as abandoned, re-verified right before the destroy); `DuplicateName` (synchronous or through the lease) re-runs the probe; lowest-MOID convergence after the import (a call whose own object is not the lowest destroys its own, then reuses the survivor after the probe) and a template is handed out only while its name is unique in the folder (otherwise, and when the name does not settle, `imageartifact.InProgressError`, which the Slice 2 manager keeps out of its circuit breaker); `prepared_image_id` is the absolute inventory path, checked to resolve (`FindByInventoryPath`) to the verified template; the artifact echo is the stamp; a per-call finder, so concurrent prepares do not race on the shared one; an unfinished import this call created is destroyed on failure on a detached context.
...
- `internal/providers/vsphere/server.go`: `GetCapabilities` advertises `SupportsImageArtifactIdentity`.
...
- Tests (vcsim): fresh stamped import and echo, reuse without download, new UID/digest → new artifact, conflicts left untouched (unstamped, other UID/digest, untrusted stamp, unfinished foreign VM), unfinished artifacts (fresh, running task, `Destroy_Task` blocked, `prepare.timeout`-derived bound, powered on, abandoned → re-import), import-folder scope and unusable folders, multi-VM OVF (a `VirtualSystemCollection`, and a `VirtualAppImportSpec`), permanent vs transient failures, URL redaction, an OVF-forged `virtrigaud.image.*` stripped, a characterization test pinning that vcsim's `ImportVApp` does not return `DuplicateName`, an emulated vCenter `DuplicateName` re-probe (reuse / in progress / conflict), lowest-MOID convergence both ways and under 4 concurrent prepares, `Create`/`Clone` clearing the stamp, `Create` cloning the verified artifact by inventory path next to a same-named decoy, legacy mode with the deprecation signal, the capability, and the manager transport's error typing over gRPC. Unit tests for stamp parsing, the decision table and the fault/failure classifiers.

### Changed
- `internal/providers/vsphere/image.go`: `ImagePrepare` decodes requests with `imageartifact.ParseRequest` before any vCenter call: an identity request goes to the new path; an identity-less request from an older manager is served by `imagePrepareLegacy` (the pre-ADR bare-name path, unchanged: bare-name gate across the datacenter, `prepared_image_id` = bare name, no echo, unstamped) and emits `imageartifact.SignalLegacyRequest` (deprecated; refused from the next release). A legacy target name must now be a DNS-1123 subdomain, as every `VMImage` name is.
- `internal/providers/vsphere/ova_import.go`: in identity mode `importOVA` appends the image stamp to the import spec after `stripReservedExtraConfig` and before `ImportVApp`, refuses an OVF that is not exactly one VM as `InvalidSpec`, classifies an unreadable/invalid OVF or one vCenter's OVF parser rejects as `InvalidSpec`, maps `DuplicateName` to a re-probe, and returns the created entity on a late failure so the caller destroys its own object.
- `internal/providers/vsphere/image.go`: download failures are classified — a 4xx other than 408/429 is `InvalidSpec`, a 5xx/408/429, a transport error or a source that breaks off is `Unavailable`, a local staging failure a provider error; an unreadable OVA archive is `InvalidSpec`. OVA URLs appear in logs and messages only without user-info, query or fragment. The `.ovf`/`.ova` decision uses the URL path, so a presigned query no longer hides the extension.
...
- Docs: `docs/image-preparation.md` (new "vSphere: identity-safe prepared templates" section: naming, import folder, stamp, reuse table, liveness, concurrency and the pending lab check, failure classification, legacy mode, legacy templates, privileges; "Provider support" lists vSphere), `docs/vm-ownership.md` (legacy vs identity `ImagePrepare`, OVF-forged image stamps, clones clear the image stamp, no new privilege, reference-style known limitation → Slice 7), `docs/upgrading.md` (vSphere visible-change row, old-manager rollback caveat), `docs/adr/0009-prepared-image-artifact-identity.md` and `docs/adr/README.md` (status: Slice 3 implemented, merge gated on the lab check).

### Why
ADR-0009 closes cross-tenant template poisoning and disclosure on vSphere: a prepared template is named after, stamped with and reused only for the `VMImage` UID and source digest it was built for, in one folder, and anything else at the name is refused. A synchronous import that can never succeed now tells the manager so (`InvalidSpec`), so the image is held instead of re-downloaded every few seconds.

### Impact
- [ ] Breaking change
- [x] Requires cluster rollout (vSphere provider image; takes effect with the Slice 2 manager)
- [ ] Config change only
- [ ] Documentation only
