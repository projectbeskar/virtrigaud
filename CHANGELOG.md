...
- Docs: `docs/image-preparation.md` (new "vSphere: identity-safe prepared templates" section: naming, import folder, stamp, reuse table, liveness, concurrency and the pending lab check, failure classification, legacy mode, legacy templates, privileges), `docs/vm-ownership.md` (legacy vs identity `ImagePrepare`, OVF-forged image stamps, clones clear the image stamp, no new privilege, reference-style known limitation → Slice 7).
