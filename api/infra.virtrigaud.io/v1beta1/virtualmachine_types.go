...
type PlacementStatus struct {
	...
	// this VM does not own is there), which proves this VM created nothing on
	// it; its pendingHost is then cleared and the VM is re-scheduled onto
	// another Host. The list is bounded (the oldest entry is dropped when it is
	// full) and is cleared when the VM is bound. If every candidate Host is
